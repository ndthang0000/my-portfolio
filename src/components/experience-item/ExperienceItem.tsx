...
const ExperienceItem = ({ image, from, to, name, position, website }: ExperienceItemProps) => {
  ...
  return (
    <div className={className} onMouseOver={handleHover}>
      <div className='flex items-center h-full relative'>
        <p className='absolute top-2 right-4 text-sm italic text-[#dadde3]'>{from} - {to}</p>
        <Image
          src={image}
          width={120}
          height={120}
        ...
        <div>
          ...
          {website &&
            <Link href={website} target='_blank' className='cursor-pointer md:px-4 px-2 py:1 md:py-2 border w-52 text-center mt-2 flex items-center gap-2 justify-center hover:border-primary'>
              <GlobeIcon size={28} />
